...
public class DB {
    ...
    public static void insertime(Context context,String item_name,String time,String item_name_check ){
        DBHelper dbHelper = new DBHelper(context);
        SQLiteDatabase db = dbHelper.getWritableDatabase();

        String sql = "INSERT INTO " + DBHelper.TABLE_NAME_TIME + "(" + DBHelper.COL_ITEM_NAME_TIME + ", " + DBHelper.COL_ITEM_NAME_TIME_CHECK + ", " + DBHelper.COL_TIME +  ") VALUES ('" + item_name + "', '" + item_name_check +"', '" + time + "')";
        db.execSQL(sql);
    ...
    public static String[] selectime(Context context,String name){
        DBHelper dbHelper = new DBHelper(context);
        SQLiteDatabase db = dbHelper.getWritableDatabase();

        Cursor cursor =  db.rawQuery("SELECT * FROM " + DBHelper.TABLE_NAME_TIME + " WHERE "+ DBHelper.COL_ITEM_NAME_TIME_CHECK + "= ? ", new String[]{name});
        ...
        while (!cursor.isAfterLast()){
            ...
            res[2] = cursor.getString(cursor.getColumnIndex(DBHelper.COL_TIME));
            cursor.moveToNext();
        }
